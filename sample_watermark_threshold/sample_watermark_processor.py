...
class SampleWatermark:
    def __init__(
        ...
        n_sample_per_token_above_entropy_threshold: int = 2,
        ...
        self.n_sample_per_token_above_entropy_threshold = n_sample_per_token_above_entropy_threshold
    ...
    def generate_with_watermark(self, input_ids, args, model, device, tokenizer):
        ...
        with torch.no_grad():
            ...
            for _ in range(args.max_new_tokens):
                ...
                if args.use_entropy_threshold:
                    if next_token_entropy < args.entropy_threshold:
                        next_token_n_sample = self.n_sample_per_token
                    else:
                        next_token_n_sample = self.n_sample_per_token_above_entropy_threshold
                else:
                    next_token_n_sample = self.n_sample_per_token

                for j in range(next_token_n_sample+1):
...
class SampleWatermarkDetector():
    ...
    def detect(self, input_text, args, device, tokenizer):
        prompt = input_text
        input_ids = tokenizer.encode(prompt, return_tensors="pt", add_special_tokens=True, truncation=True, max_length=args.prompt_max_length).to(device)
